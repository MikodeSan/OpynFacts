...
class ZProduct(models.Model):
    code = models.BigIntegerField(
        "Code", primary_key=True, default=0, null=False, unique=True
    )
    ...
    nutrition_score_beverage = models.SmallIntegerField(
        "Nutrition score beverage", null=True
    )
    ...
    last_modified_t = models.BigIntegerField(
        "Date de mise à jour", null=False, default=0
    )
    # last_modified = models.DateTimeField(auto_now_add=True)
    update_t = models.DateTimeField(
        "Date d'enregistrement", auto_now_add=True, null=True
    )
    image_url = models.URLField("Image URL", null=True)

    categories = models.ManyToManyField(
        "ZCategory", through="ZCategory_Product", related_name="products", blank=True
    )
    searchers = models.ManyToManyField(
        get_user_model(), through="ZSearch", related_name="searches", blank=True
    )
    users = models.ManyToManyField(
        get_user_model(), through="ZFavorite", related_name="favorites", blank=True
    )
    alternatives = models.ManyToManyField(
        "self", related_name="substituted", symmetrical=False, blank=True
    )
...
class ZSearch(models.Model):
    ...
    favorite_alternatives = models.ManyToManyField(
        ZFavorite, related_name="zsearches", blank=True
    )
