...
class Command(BaseCommand):
    ...
    def init_product_db(self, category_lst):
        ...
        for product_db in product_lst_db:
            ...
            product_response_dct = source.get_product(
                str(product_db.code), locale=self.LOCALE
            )
        ...
        for idx, category_dct in enumerate(category_lst):
            ...
            product_lst = source.get_products_from_category(
                category_url, self.N_PRODUCT_MAX
            )
            print(
                "\t> {}. Process category #{} - {} Products".format(
                    idx, category_id, len(product_lst)
                )
            )
            ...
            for product_dct in product_lst[: self.N_HEALTHY_MAX]:
                ...
                if not any(
                    product_dct["code"] in dct.values() for dct in best_product_lst
                ):
...
def update_product_db(data_dct, force=False):
    ...
    if force or (
        not force and product_targeted.last_modified_t < data_dct["last_modified_t"]
    ):
        ...
        for idx, category_id in enumerate(categories_hierarchy_lst):
            ...
            category_db, created = ZCategory.objects.get_or_create(
                identifier=category_id
            )
            # print(category_db, created)
            product_targeted.categories.add(
                category_db, through_defaults={"hierarchy_index": idx}
            )
