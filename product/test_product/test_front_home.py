...
class TestFrontHomeAnomymous(StaticLiveServerTestCase):
    ...
    def test_other_2_home(self):
        ...
        WebDriverWait(driver, DEFAULT_TIMEOUT_S).until(
            lambda drv: drv.find_element_by_id("notice_section_id")
        )
        ...
        home_link.click()
        WebDriverWait(driver, DEFAULT_TIMEOUT_S).until(
            lambda drv: drv.find_element_by_id("history")
        )
    ...
    def test_home_2_signin_page(self):
        ...
        WebDriverWait(driver, DEFAULT_TIMEOUT_S).until(
            lambda drv: drv.find_element_by_id("signin_a")
        )

        singin_link = driver.find_element_by_id("signin_a")
        singin_link.click()
        WebDriverWait(driver, DEFAULT_TIMEOUT_S).until(
            lambda drv: drv.find_element_by_id("contact")
        )
    ...
    def test_nav_search_bar_2_result_page(self):
        ...
        WebDriverWait(driver, SEARCH_TIMEOUT_S).until(
            lambda drv: drv.find_element_by_id("product_section")
        )

        self.assertEqual(
            driver.current_url,
            build_full_url(self, "product:result", {"user_query": query}),
        )
    ...
    def test_home_search_bar_2_result_page(self):
        ...
        # Set the query
        masthead = WebDriverWait(driver, DEFAULT_TIMEOUT_S).until(
            EC.visibility_of_element_located((By.CLASS_NAME, "masthead"))
        )
    ...
    def test_home_2_notice_page(self):
        ...
        notice_link = WebDriverWait(driver, DEFAULT_TIMEOUT_S).until(
            EC.element_to_be_clickable((By.ID, "notice_a"))
        )
...
class TestFrontHomeAuthenticated(StaticLiveServerTestCase):
    ...
    def setUp(self):
        ...
        # Sign-in user
        sign_in_user_into_webdriver(
            self, self.USER, self.USER_PWD, self.WEB_DRIVER, "product:home"
        )
    ...
    def test_home_to_profil(self):
        ...
        time.sleep(1)
        WebDriverWait(driver, DEFAULT_TIMEOUT_S).until(
            lambda drv: drv.find_element_by_id("profile_section_id")
        )
